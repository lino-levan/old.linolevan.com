...
const Header = () => {
  let links = [{name:"~/projects", url:"/projects"}, {name:"~/posts", url:"/posts"}]
      ...
      <div className="mx-auto flex w-full max-w-6xl justify-between">
        <div className="navbar flex items-center space-x-3 text-xs sm:space-x-8 sm:text-lg">
          ...
          <div>
            {
              links.map(({name, url})=>(
                <span key={name} className="pl-4 text-emerald-500 hover:text-teal-800"><Link href={url}>{name}</Link></span>
              ))
            }
          </div>
        </div>
