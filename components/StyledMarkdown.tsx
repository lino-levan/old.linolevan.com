    ...
    <ReactMarkdown
      ...
      components={{
        a({ node, children, href, ...props }) {
          return (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="bg-opacity-30 text-emerald-600 bg-emerald-300 hover:bg-emerald-400 hover:bg-opacity-30"
            >
              {children}
            </a>
          );
        ...
        blockquote({ node, children, ...props }) {
          return (
            <blockquote className="border-l pl-4 italic">{children}</blockquote>
          );
        },
        code({ node, children, ...props }) {
          return (
            <code className="bg-slate-100 p-1 rounded shadow">{children}</code>
          );
        },
        pre({ node, children, ...props }) {
          let raw = (children?.[0] as any)?.props?.children?.[0];
          return (
            <pre className="bg-slate-100 p-1 rounded shadow overflow-auto">{raw?raw:children}</pre>
          );
        },
        img({ node, children, src, alt, ...props }) {
          ...
          return (
            <img
              src={src?.includes("https") ? src : ("/posts" + src)}
              alt={alt}
            />
          );
        },
